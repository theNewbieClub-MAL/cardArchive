...
ForEach ($index in $jsonIndex) {
    ...
    ForEach ($item in $index.editions) {
        ...
        ForEach ($staff in $item.contributors) {
            ...
            Switch ($status) {
                "invalid"  { $message = " &mdash; ⚠️ No cards from this staff were archived due to the link is invalid/missing" }
                "progress" { $message = " &mdash; 💳 Cards are being created by staff" }
                "halted"   { $message = " &mdash; 🚪 Staff left before finishing the requests" }
                "unknown"  { $message = " &mdash; ❓ Cards were not archived due to status is unknown" }
                Default    { $message = "" }
            }
